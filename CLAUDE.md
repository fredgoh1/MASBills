...
- **Callout formatting:** Auction results are posted using Roam's callout syntax. The parent block uses `> [!Summary]+ **title**` and each child block is prefixed with `> ` so they render as callout content without bullet points.
